export const CAREER_TEMPLATES = {
  "Software Engineer": {
    title: "Software Engineer Path",
    description: "Master the art of building software applications.",
    steps: [
      { title: "Learn HTML/CSS/JS", description: "The building blocks of the web." },
      { title: "Master React", description: "Build dynamic user interfaces." },
      { title: "Learn Backend (Node/Convex)", description: "Handle data and server logic." },
      { title: "Build a Portfolio", description: "Showcase your projects." },
      { title: "Apply for Jobs", description: "Start your career." },
    ]
  },
  "Data Scientist": {
    title: "Data Scientist Path",
    description: "Extract insights from data to drive decisions.",
    steps: [
      { title: "Learn Python", description: "The primary language for data science." },
      { title: "Master SQL", description: "Query and manage databases." },
      { title: "Learn Pandas & NumPy", description: "Data manipulation libraries." },
      { title: "Machine Learning Basics", description: "Understand algorithms and models." },
      { title: "Build Data Projects", description: "Analyze real-world datasets." },
    ]
  },
  "Product Manager": {
    title: "Product Manager Path",
    description: "Lead product development and strategy.",
    steps: [
      { title: "Understand Agile", description: "Learn software development methodologies." },
      { title: "User Research", description: "Understand user needs and pain points." },
      { title: "Roadmapping", description: "Plan product features and timelines." },
      { title: "Data Analysis", description: "Make data-driven decisions." },
      { title: "Stakeholder Management", description: "Communicate effectively with teams." },
    ]
  },
  "UX Designer": {
    title: "UX Designer Path",
    description: "Design intuitive and user-friendly experiences.",
    steps: [
      { title: "Learn Design Principles", description: "Color, typography, and layout." },
      { title: "Master Figma", description: "The industry standard design tool." },
      { title: "User Research", description: "Understand user behaviors." },
      { title: "Prototyping", description: "Create interactive mockups." },
      { title: "Build a Portfolio", description: "Showcase your design process." },
    ]
  }
};
